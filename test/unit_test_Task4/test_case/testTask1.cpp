...
bool UNIT_TEST_Task4::Task4005()
{
   string name = "Task4101";
    //! data ------------------------------------
    UnitList *list = new UnitList(243);

    Unit **unitArray = new Unit *[13];
    for (int i = 0; i <= 6; ++i)
    {
        unitArray[i] = new Vehicle(1, 1, Position(i, i), VehicleType(i));
    }
    for (int i = 0; i <= 5; ++i)
    {
        unitArray[i + 7] = new Infantry(1, 1, Position(i, i), InfantryType(i));
    }

    bool insertResults[13];
    for (int i = 0; i < 13; i++)
    {
        insertResults[i] = list->insert(unitArray[i]);
    }
    ...
    for (int i = 0; i < 12; i++)
    {
        result = result && insertResults[i];
    }
    // The 13th insertion should fail
    result = result && insertResults[12];
